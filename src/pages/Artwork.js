import Masonry, {ResponsiveMasonry} from "react-responsive-masonry"

const Artwork = () => {
  return (
    <>
      <div class="main-column">

        <div class="cardContainerPurple">
              <div class="titleWrapPurple">
                  <h2>Hellscape Digital / Artwork</h2>
              </div>            
              <div class="containedText">
                  <h3>
                      By Artist
                  </h3>                
              </div>
              <div class="flexColumn">
                    <div class="containedText">
                        <p>
                            ANJU Online!<br/>
                        </p>
                    </div>
                    <div class="containedText">
                        <p>
                            Cloudi Daze<br/>
                        </p>
                    </div>
                </div>
          </div>

        <div class="cardContainerBlue">
              <div class="titleWrapBlue">
                  <h2>Artwork / ANJU Online!</h2>
              </div>            
              <div class="masonry-grid-art">
                <div class="gallery-item-art">
                    <img src="https://f8n-production-collection-assets.imgix.net/0x1f597E08f4e7428845EB693d9F49B33f07eF04eD/1/nft.png?q=80&auto=format%2Ccompress&cs=srgb&max-w=1680&max-h=1680" width="100%"/><br/>
                </div>    
                <div class="gallery-item-art">
                    <img src="https://pbs.twimg.com/media/FATYxC0WEAUb4r1?format=jpg&name=large" width="100%"/><br/>
                </div> 
                <div class="gallery-item-art">
                    <img src="https://images-wixmp-ed30a86b8c4ca887773594c2.wixmp.com/f/24b62acf-0cd7-4009-93ed-a0d140e5170d/deorq4y-cd16c7fd-66ac-43b6-996a-468c8289b99d.jpg?token=eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..GndpeiU8ZI1OUrAj8y3XaZeWNezseCozE0PIoGjt3IM" width="100%"/><br/>
                </div>  
                <div class="gallery-item-art">
                    <img src="https://pbs.twimg.com/media/FRnSLHeXwAMvnoZ?format=jpg&name=4096x4096" width="100%"/><br/>
                </div>  
                <div class="gallery-item-art">
                    <img src="https://images-wixmp-ed30a86b8c4ca887773594c2.wixmp.com/f/24b62acf-0cd7-4009-93ed-a0d140e5170d/deonync-8e189aef-e371-45e5-bd50-033b6eba9122.gif?token=eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..Xe4G1EgO1MO6F7ri6YNGjMDYumIcguQmeu1aM7fmPPc" width="100%"/><br/>
                </div>  
                <div class="gallery-item-art">
                    <img src="https://images-wixmp-ed30a86b8c4ca887773594c2.wixmp.com/f/24b62acf-0cd7-4009-93ed-a0d140e5170d/deorpt1-a72d0361-b1b2-4533-954c-d4ffe385b912.jpg?token=eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..UlcHUb1ZfmpbhVLViu4AUHBccVZ4s10pHGn84xkQGwk" width="100%"/><br/>
                </div>                                                                
            </div> 
            <p>Full Gallery</p>
          </div>

          <div class="cardContainerGreen">
              <div class="titleWrapGreen">
                  <h2>Artwork / ANJU Online! (Test)</h2>
              </div> 
              <div class="masonry-container">           
                <div class="masonry-grid-test">
                    <img src="https://f8n-production-collection-assets.imgix.net/0x1f597E08f4e7428845EB693d9F49B33f07eF04eD/1/nft.png?q=80&auto=format%2Ccompress&cs=srgb&max-w=1680&max-h=1680"/>
                </div>   
                <div class="masonry-grid-test">      
                    <img src="https://pbs.twimg.com/media/FATYxC0WEAUb4r1?format=jpg&name=large"/>
                </div>
                <div class="masonry-grid-test">                
                    <img src="https://images-wixmp-ed30a86b8c4ca887773594c2.wixmp.com/f/24b62acf-0cd7-4009-93ed-a0d140e5170d/deorq4y-cd16c7fd-66ac-43b6-996a-468c8289b99d.jpg?token=eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..GndpeiU8ZI1OUrAj8y3XaZeWNezseCozE0PIoGjt3IM"/>
                </div>   
                <div class="masonry-grid-test">      
                    <img src="https://pbs.twimg.com/media/FRnSLHeXwAMvnoZ?format=jpg&name=4096x4096"/>
                </div>
                <div class="masonry-grid-test">   
                    <img src="https://images-wixmp-ed30a86b8c4ca887773594c2.wixmp.com/f/24b62acf-0cd7-4009-93ed-a0d140e5170d/deonync-8e189aef-e371-45e5-bd50-033b6eba9122.gif?token=eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..Xe4G1EgO1MO6F7ri6YNGjMDYumIcguQmeu1aM7fmPPc"/>  
                </div>                
                <div class="masonry-grid-test">         
                    <img src="https://images-wixmp-ed30a86b8c4ca887773594c2.wixmp.com/f/24b62acf-0cd7-4009-93ed-a0d140e5170d/deorpt1-a72d0361-b1b2-4533-954c-d4ffe385b912.jpg?token=eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..UlcHUb1ZfmpbhVLViu4AUHBccVZ4s10pHGn84xkQGwk"/>                                                                
                </div>
            </div>
            <p>Full Gallery</p>
          </div>

          <div class="cardContainerYellow">
              <div class="titleWrapYellow">
                  <h2>Artwork / ANJU Online! (React Test)</h2>
              </div> 
              <div class="masonryWrap">
                <Masonry columnsCount={2} gutter={"0.75em"}>
                    <img src="https://f8n-production-collection-assets.imgix.net/0x1f597E08f4e7428845EB693d9F49B33f07eF04eD/1/nft.png?q=80&auto=format%2Ccompress&cs=srgb&max-w=1680&max-h=1680"/>
                    <img src="https://pbs.twimg.com/media/FRnSLHeXwAMvnoZ?format=jpg&name=4096x4096"/>
                    <img src="https://pbs.twimg.com/media/FATYxC0WEAUb4r1?format=jpg&name=large"/>
                    <img src="https://images-wixmp-ed30a86b8c4ca887773594c2.wixmp.com/f/24b62acf-0cd7-4009-93ed-a0d140e5170d/deonync-8e189aef-e371-45e5-bd50-033b6eba9122.gif?token=eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..Xe4G1EgO1MO6F7ri6YNGjMDYumIcguQmeu1aM7fmPPc"/>
                    <img src="https://images-wixmp-ed30a86b8c4ca887773594c2.wixmp.com/f/24b62acf-0cd7-4009-93ed-a0d140e5170d/deorq4y-cd16c7fd-66ac-43b6-996a-468c8289b99d.jpg?token=eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..GndpeiU8ZI1OUrAj8y3XaZeWNezseCozE0PIoGjt3IM"/>
                    <img src="https://images-wixmp-ed30a86b8c4ca887773594c2.wixmp.com/f/24b62acf-0cd7-4009-93ed-a0d140e5170d/deorpt1-a72d0361-b1b2-4533-954c-d4ffe385b912.jpg?token=eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..UlcHUb1ZfmpbhVLViu4AUHBccVZ4s10pHGn84xkQGwk"/>
                </Masonry>
              </div>
              <p>Full Gallery</p>
            </div>

          </div>
    </>
  );
};
export default Artwork;